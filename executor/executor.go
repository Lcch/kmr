...
func (cw *ComputeWrap) doReduce(bk bucket.Bucket, reduceID int, nMap int, commitMappers []int64, writer records.RecordWriter) error {
	...
	for r := range sorted {
		...
		<-waitReduce
		inputs = make(chan *kmrpb.KV, 1024)
		waitReduce = cw.doReduceForSingleKey(inputs, writer)
		inputs <- RecordToKV(r)
	}
	close(inputs)
	<-waitReduce
...
func (cw *ComputeWrap) doReduceForSingleKey(inputs <-chan *kmrpb.KV, writer records.RecordWriter) chan struct{} {
	waitc := make(chan struct{})
	outputKV := cw.reduceFunc(inputs)
	go func() {
		for range outputKV {
